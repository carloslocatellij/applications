...
if 0==1:
    from gluon import *
    from gluon import db, IS_IN_SET, IS_UPPER, IS_EMPTY_OR, IS_IN_DB, IS_NOT_IN_DB, IS_MATCH, a_db, auth, Auth, buscador
...
def Modal(title, content, id, vbutton=False):
    ...
    modal =  CAT(XML('<button class="btn btn-primary" data-toggle="up-modal" data-target="#{}">{}</button>'.format(id, vbutton)) if vbutton else '',
        DIV(DIV(DIV(DIV(
...
def buscador(tabela, regform=request.function ,**fields):
            ...
            if session[k] != None:
                if 'table' in v:
                    ...
                    if db[tab2][k].type == 'string':
                        q.append( ( db[tabela][campo2] == db[tab2].id) & (db[tab2][k].contains(str(session[k]).strip())) )
                    elif db[tab2][k].type == 'integer':
                        q.append( ( db[tabela][campo2] == db[tab2].id) & (db[tab2][k] == int(str(session[k]).strip())) )

                elif db[tabela][k].type == 'string':
                    if k[-1] == 's':
                        try:
                            q.append((db[tabela][k[0:-1]].contains(str(session[k]) ) ) )
                        except KeyError:
                            try:
    ...
    links = [dict(header='Ver', body=lambda row: A('Ver', _class='btn btn-primary' ,
     _href=URL(c=session.controller, f=regform, args=row[tabela]['id'] if tab2 != None else row['id'], vars={'f': 'ver'})))]

    grade = SQLFORM.grid( busca, represent_none='', links=links, editable=False, searchable=False, deletable=False,
             create=False, details=False, csv=False,  maxtextlength = 120, _class="table", links_placement= 'left' ,
             user_signature=False, paginate=100 )
