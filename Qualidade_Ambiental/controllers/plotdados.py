...
# Em desenvolvimento, as próximas linhas servem apenas para adaptação do escopo de nomes do framework ao IDE
if 0==1:
    from gluon import *
    from gluon import db, IS_IN_SET, IS_UPPER, IS_EMPTY_OR, IS_IN_DB, IS_NOT_IN_DB, IS_MATCH, a_db, db, auth, Auth, pegaDof, EntradaPonto
...
from plotador import * #este modulo se encontra na pasta modules . É o gerador de graficos
import datetime
...
#df_mysql = pd.DataFrame(query.values()) SE FOR USAR PANDAS, MAS É PESADO
# SEM USO
...
def voldia(): # Grafico de volume por dia
    ...
    if session.IdPonto:
        DadosPonto = db((EntradaPonto.Data>=session.Inicio) & (EntradaPonto.Data<session.Fim) & (EntradaPonto.IdPonto==session.IdPonto))
    else:
    ...
    for v in query:
        x.append(datetime.date.strftime(query.get(v).get('EntradaPonto').get('Data'), '%d/%m'))
    for v in query:
        y.append(int(query.get(v).get('Volume')))
    lxy = []
    for i in zip(x,y):
        lxy.append(i)
    legendgrau = 300
...
def graf_entra_att():
    ...
    ylab = u'Vol. em metros cúbicos (m3)'
    xlab = u'Atts'
    title = u'Volume de Resíduos '
    (x , y) = ([], [])
    lxy = []

    for i in query:
        x.append(i['EntradaATT.Unid_Destino']) #.encode('utf-8'))
        for k,v in i.items():
            if v != None:
                y.append(v)
    for i in zip(x,y):
    ...
    return plot( title=title, xlab=xlab, ylab=ylab,  mode='bar', data={u'Volume por ATT.': lxy}, legendgrau=legendgrau)
