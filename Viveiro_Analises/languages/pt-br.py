# -*- coding: utf-8 -*-
{
'': '',
'!=': '!=',
'!langcode!': 'pt-br',
'!langname!': 'Português (do Brasil)',
'"update" is an optional expression like "field1=\'newvalue\'". You cannot update or delete the results of a JOIN': '"update" é uma expressão opcional como "campo1=\'novovalor\'". Você não pode atualizar ou apagar os resultados de um JOIN',
'%(nrows)s records found': '%(nrows)s registro(s) encontrado(s)',
'%d/%m/%Y': '%d/%m/%Y',
'%d/%m/%Y %H:%M:%S': '%d/%m/%Y %H:%M:%S',
'%s %%{row} deleted': '%s linha apagadas',
'%s %%{row} updated': '%s linha atualizadas',
'%s selected': '%s selecionado',
'%Y-%m-%d': '%d-%m-%Y',
'%Y-%m-%d %H:%M:%S': '%d-%m-%Y %H:%M:%S',
'(**%.0d MB**)': '(**%.0d MB**)',
'**%(items)s** %%{item(items)}, **%(bytes)s** %%{byte(bytes)}': '**%(items)s** %%{item(items)}, **%(bytes)s** %%{byte(bytes)}',
'**%(items)s** items, **%(bytes)s** %%{byte(bytes)}': '**%(items)s** items, **%(bytes)s** %%{byte(bytes)}',
'**not available** (requires the Python [[guppy http://pypi.python.org/pypi/guppy/ popup]] library)': '**not available** (requires the Python [[guppy http://pypi.python.org/pypi/guppy/ popup]] library)',
'+ And': '+ E',
'+ Or': '+ Ou',
'1ª Especie retirada': '1ª Especie retirada',
'<': '<',
'<=': '<=',
'=': '=',
'>': '>',
'>=': '>=',
'?': '?',
'@markmin\x01An error occured, please [[reload %s]] the page': 'Ocorreu um erro, por favor [[reload %s]] a página',
'@markmin\x01Number of entries: **%s**': 'Number of entries: **%s**',
'[Wiki]': '[Wiki]',
'``**not available**``:red (requires the Python [[guppy http://pypi.python.org/pypi/guppy/ popup]] library)': '``**not available**``:red (requires the Python [[guppy http://pypi.python.org/pypi/guppy/ popup]] library)',
'A3P': 'A3P',
'A3P ': 'A3P ',
'A3P Energia': 'A3P Energia',
'About': 'Sobre',
'Access Control': 'Controle de Acesso',
'Add Record': 'Add Registro',
'Add record to database': 'Add registro a base de dados',
'Add this to the search as an AND term': 'Adicionar termo a busca como um AND termo',
'Add this to the search as an OR term': 'Adicionar termo a busca como um OR termo',
'admin': 'admin',
'Administrative Interface': 'Interface Administrativa',
'Administrative interface': 'Interface administrativa',
'Ajax Recipes': 'Receitas de Ajax',
'Alterar': 'Alterar',
'Alterar dados da foto': 'Alterar dados da foto',
'Alterar foto': 'Alterar foto',
'Ameaca': 'Ameaca',
'An error occured, please [[reload %s]] the page': 'An error occured, please [[reload %s]] the page',
'An error occured, please reload the page': 'An error occured, please reload the page',
'API Example': 'API de Exemplo',
'appadmin is disabled because insecure channel': 'Administração desativada porque o canal não é seguro',
'Apply changes': 'Mudanças Aplicadas',
'Are you sure you want to delete this object?': 'Você tem certeza que quer apagar este objeto?',
'Area': 'Área',
'Atualizar Bairro': 'Atualizar Bairro',
'Atualizar despacho_templat': 'Atualizar despacho_templat',
'Atualizar Especie': 'Atualizar Especie',
'Atualizar foto': 'Atualizar foto',
'Atualizar Laudo': 'Atualizar Laudo',
'Atualizar Modelo': 'Atualizar Modelo',
'Atualizar Requerimento': 'Atualizar Requerimento',
"Atualizar {'Requerimento'}": "Atualizar {'Requerimento'}",
'Available Databases and Tables': 'Bancos de dados e tabelas disponíveis',
'Back': 'Volta',
'Bairro': 'Bairro',
'Bairro não registrado.': 'Bairro não registrado.',
'Bioma': 'Bioma',
'Body': 'Corpo',
'Buy this book': 'Compre o livro',
"Buy web2py's book": 'Compre o livro do web2py',
'cache': 'cache',
'Cache': 'Cache',
'Cache Cleared': 'Cache limpo',
'Cache contains items up to **%(hours)02d** %%{hour(hours)} **%(min)02d** %%{minute(min)} **%(sec)02d** %%{second(sec)} old.': 'Cache contains items up to **%(hours)02d** %%{hour(hours)} **%(min)02d** %%{minute(min)} **%(sec)02d** %%{second(sec)} old.',
'Cache Keys': 'Chaves de cache',
'Calcada': 'Calcada',
'Cannot be empty': 'Não pode estar vazio',
'Cas': 'Cas',
'Cep': 'Cep',
'CEP': 'CEP',
'Change Password': 'Trocar senha',
'change password': 'mudar senha',
'Change password': 'Change password',
'Changelog': 'Changelog',
'Check to delete': 'Marque para apagar',
'Choose Template or empty for new Page': 'Choose Template or empty for new Page',
'Classesucessao': 'Classesucessao',
'Clear': 'Limpar',
'Clear CACHE?': 'Limpar CACHE?',
'Clear DISK': 'Limpar DISCO',
'Clear RAM': 'Limpar memória RAM',
'Click on the link %(link)s to reset your password': 'Click on the link %(link)s to reset your password',
'Client IP': 'IP do cliente',
'Close': 'Fecha',
'Cod': 'Cod',
'Codagua': 'Codagua',
'Codcpfl': 'Codcpfl',
'Coddepto': 'Coddepto',
'Codenergia': 'Codenergia',
'Codsemae': 'Codsemae',
'Comma-separated export including columns not shown; fields from other tables are exported as raw values for faster export': 'Comma-separated export including columns not shown; fields from other tables are exported as raw values for faster export',
'Comma-separated export of visible columns. Fields from other tables are exported as they appear on-screen but this may be slow for many rows': 'Comma-separated export of visible columns. Fields from other tables are exported as they appear on-screen but this may be slow for many rows',
'Community': 'Comunidade',
'Components and Plugins': 'Componentes e Plugins',
'Config.ini': 'Config.ini',
'Confirm Password': 'Confirm Password',
'Conflito com fiação elétrica': 'Conflito com fiação elétrica',
'Consumo': 'Consumo',
'contains': 'contêm',
'Controller': 'Controlador',
'Copyright': 'Copyright',
'Cordaflor': 'Cordaflor',
'Cpf': 'Cpf',
'Cpf Cnpj': 'Cpf Cnpj',
'CPF/CNPJ': 'CPF/CNPJ',
'Create New Page': 'Criar Nova Página',
'Create Page from Slug': 'Create Page from Slug',
'Created By': 'Criado por',
'Created On': 'Criado em',
'CSV': 'CSV',
'CSV (hidden cols)': 'CSV (hidden cols)',
'Current request': 'Requisição atual',
'Current response': 'Resposta atual',
'Current session': 'Sessão atual',
'customize me!': 'personalize-me!',
'Danos à estrutura da construção': 'Danos à estrutura da construção',
'Dap Max': 'Dap Max',
'Data': 'Data',
'Data de Entrada': 'Data de Entrada',
'Data Do Laudo': 'Data Do Laudo',
'Data do Laudo': 'Data do Laudo',
'Data Entrada': 'Data Entrada',
'data uploaded': 'dados enviados',
'Database': 'banco de dados',
'Database %s select': 'Selecionar banco de dados %s',
'Database Administration (appadmin)': 'Administração da Base de Dados (appadmin)',
'Datareg': 'Data de registro',
'Datavalidade': 'Data de validade',
'db': 'bd',
'DB Model': 'Modelo BD',
'Delete': 'Delete',
'Delete:': 'Apagar:',
'Demo': 'Demo',
'Denuncia': 'Denuncia',
'Deployment Recipes': 'Receitas de deploy',
'Description': 'Descrição',
'Descrição': 'Descrição',
'design': 'projeto',
'Design': 'Projeto',
'Despacho': 'Despacho',
'deve ser imagem no formato jpeg ou png': 'deve ser imagem no formato jpeg ou png',
'Deve ter o formato xx/xx/20xx': 'Deve ter o formato xx/xx/20xx',
'DISK': 'DISCO',
'Disk Cache Keys': 'Chaves do Cache de Disco',
'Disk Cleared': 'Disco Limpo',
'DISK contains items up to **%(hours)02d** %%{hour(hours)} **%(min)02d** %%{minute(min)} **%(sec)02d** %%{second(sec)} old.': 'DISK contains items up to **%(hours)02d** %%{hour(hours)} **%(min)02d** %%{minute(min)} **%(sec)02d** %%{second(sec)} old.',
'Documentation': 'Documentação',
"Don't know what to do?": 'Não sabe o que fazer?',
'done!': 'concluído!',
'Download': 'Download',
'E-mail': 'E-mail',
'Edit': 'Editar',
'Edit current record': 'Editar o registro atual',
'Edit Menu': 'Editar Menu',
'Edit Page': 'Editar Página',
'Edit Page Media': 'Editar Media da Página',
'edit profile': 'Editar perfil',
'Edit This App': 'Editar esta aplicação',
'Email': 'Email',
'Email and SMS': 'Email e SMS',
'Email sent': 'Envio de Email',
'Email verification': 'Verificação de Email',
'Email verified': 'Email verificado',
'Endereco1': 'Endereco',
'Endereço': 'Endereço',
'Enter a number between %(min)g and %(max)g': 'Informe um número entre %(min)g e %(max)g',
'Enter a value': 'Inserir valor',
'Enter an integer between %(min)g and %(max)g': 'Informe um valor inteiro entre %(min)g e %(max)g',
'Enter date and time as %(format)s': 'Informe uma data e hora no formato: %(format)s',
'Enter date as %(format)s': 'Informe uma data no formato %(format)s',
'Enter from %(min)g to %(max)g characters': 'Informe de %(min)g até %(max)g caracteres',
'Enviar': 'Enviar',
'Errors': 'Erros',
'Errors in form, please check it out.': 'Erros no formulário, porfavor verifica-lo novamente..',
'Especie': 'Especie',
'Especie Poda1': 'Especie Poda1',
'Especie Poda2': 'Especie Poda2',
'Especie Poda3': 'Especie Poda3',
'Especie Poda4': 'Especie Poda4',
'Especie Ret1': 'Especie Ret1',
'Especie Ret2': 'Especie Ret2',
'Especie Ret3': 'Especie Ret3',
'Especie Ret4': 'Especie Ret4',
'Estoque': 'Estoque',
'export as csv file': 'exportar como um arquivo csv',
'Export:': 'Exportar:',
'Familia': 'Familia',
'FAQ': 'Perguntas frequentes',
'file': 'arquivo',
'file ## download': 'arquivo ',
'Fimfloracao': 'Fimfloracao',
'Fimfrutificacao': 'Fimfrutificacao',
'First name': 'Nome',
'Forms and Validators': 'Formulários e Validadores',
'Free Applications': 'Aplicações gratuitas',
'Frutifera': 'Frutifera',
'Gerar Gráficos': 'Gerar Gráficos',
'Graph Model': 'Graph Model',
'Grid Example': 'Exemplo de Grade',
'Group ID': 'ID do Grupo',
'Group uniquely assigned to user %(id)s': 'Group uniquely assigned to user %(id)s',
'Groups': 'Grupos',
'Grupoeco': 'Grupoeco',
'Hello World': 'Olá Mundo',
'Helping web2py': 'Ajudar o web2py',
'Hit Ratio: **%(ratio)s%%** (**%(hits)s** %%{hit(hits)} and **%(misses)s** %%{miss(misses)})': 'Hit Ratio: **%(ratio)s%%** (**%(hits)s** %%{hit(hits)} and **%(misses)s** %%{miss(misses)})',
'Home': 'Principal',
'How did you get here?': 'Como você chegou aqui?',
'HTML': 'HTML',
'HTML export of visible columns': 'HTML exportar colunas visíveis',
'Id': 'Id',
'Idcat': 'Idcat',
'Iddepto': 'Iddepto',
'Idlocal': 'Idlocal',
'Idmaterial': 'Idmaterial',
'Idpredio': 'Idpredio',
'Idsub': 'Idsub',
'import': 'importar',
'Import/Export': 'Importar/Exportar',
'in': 'in',
'Index': 'Início',
'Inifloracao': 'Inifloracao',
'Inifrutificacao': 'Inifrutificacao',
'insert new': 'inserir novo',
'insert new %s': 'inserir novo %s',
'Insufficient privileges': 'Insufficient privileges',
'Internal State': 'Estado Interno',
'Introduction': 'Introdução',
'Invalid email': 'Email inválido',
'Invalid image': 'Invalid image',
'Invalid json': 'Invalid json',
'Invalid login': 'login inválido',
'Invalid Query': 'Consulta Inválida',
'Invalid query': 'Invalid query',
'invalid request': 'requisição inválida',
'Invalid username': 'Invalid username',
'Is Active': 'Is Active',
'JSON': 'JSON',
'JSON export of visible columns': 'JSON exportar colunas visíveis',
'Já está registrado.': 'Já está registrado.',
'Já existe um Processo Registrado com este nº de Protocolo': 'Já existe um Processo Registrado com este nº de Protocolo',
'Key': 'Chave',
'Kwh': 'Kwh',
'Last name': 'Sobrenome',
'Layout': 'Layout',
'Layout Plugins': 'Plugins de Layout',
'Layouts': 'Layouts',
'Live chat': 'Chat ao vivo',
'Live Chat': 'Chat ao vivo',
'Local': 'Local',
'Local Arvore': 'Local Arvore',
'Log In': 'Entrar',
'Logged in': 'Usuário logado',
'Logged out': 'Usuário saiu',
'login': 'Entrar',
'Login': 'Entrar',
'logout': 'Sair',
'Logout': 'Deslogar-se',
'Lost Password': 'Esqueceu sua senha?',
'lost password?': 'esqueceu sua senha?',
'Lost your password?': 'Esqueceu sua senha?',
'Main Menu': 'Menu Principal',
'Manage %(action)s': 'Gerenciar %(action)s',
'Manage Access Control': 'Gerenciar controle de acesso',
'Manage Cache': 'Gerenciar Cache',
'Manage Pages': 'Gerenciar Páginas',
'Material': 'Material',
'Memberships': 'Grupos',
'Menu Item': 'Menu Item',
'Menu Model': 'Modelo de Menu',
'Mes': 'Mes',
'Modified By': 'Modificado por',
'Modified On': 'Modificado em',
'Morador': 'Morador',
'must be YYYY-MM-DD!': 'deve ser YYYY-MM-DD!',
'My Sites': 'Meus sites',
'Name': 'Nome',
'Nativabr': 'Nativabr',
'Ndias': 'Ndias',
'New password': 'Novo password',
'New Record': 'Novo Registro',
'new record inserted': 'novo registro inserido',
'New Search': 'Nova busca',
'next %s rows': 'próximas %s ´linhas',
'next 100 rows': 'próximas 100 linhas',
'No Carteira': 'No Carteira',
'No databases in this application': 'Não há bancos de dados nesta aplicação',
'No records found': 'Nenhum registro encontrado',
'Nome': 'Nome',
'Nome do Modelo': 'Nome do Modelo',
'not authorized': 'Não autorizado',
'Not Authorized': 'Not Authorized',
'not in': 'não se encontra em',
'Number of entries: **%s**': 'Número de entradas: **%s**',
'Numero1': 'Numero1',
'Nº Carteira': 'Nº Carteira',
'Número': 'Número',
'Object or table name': 'Nome do objeto do da tabela',
'Obras, reforma, construção, demolição': 'Obras, reforma, construção, demolição',
'Obs': 'Obs',
'Old password': 'Password antigo',
'Olá  Sistema de Dados da Secretaria Municipal de Meio Ambiente - São José do Rio Preto': 'Olá  Sistema de Dados da Secretaria Municipal de Meio Ambiente - São José do Rio Preto',
'Olá ! Você está no Sistema de Dados da Sec. Municipal de Meio Ambiente - São José do Rio Preto': 'Olá ! Você está no Sistema de Dados da Sec. Municipal de Meio Ambiente - São José do Rio Preto',
'Olá ! Você está no Sistema de Dados da Secretaria Municipal de Meio Ambiente - São José do Rio Preto': 'Olá ! Você está no Sistema de Dados da Secretaria Municipal de Meio Ambiente - São José do Rio Preto',
'Olá auth_user.first_name Sistema de Dados da Secretaria Municipal de Meio Ambiente - São José do Rio Preto': 'Olá auth_user.first_name Sistema de Dados da Secretaria Municipal de Meio Ambiente - São José do Rio Preto',
'Olá Carlos! Você está no Sistema de Dados da Sec. Municipal de Meio Ambiente \n\n                           Você tem 0 avisos': 'Olá Carlos! Você está no Sistema de Dados da Sec. Municipal de Meio Ambiente \n\n                           Você tem 0 avisos',
'Olá Carlos! Você está no Sistema de Dados da Sec. Municipal de Meio Ambiente - São José do Rio Preto': 'Olá Carlos! Você está no Sistema de Dados da Sec. Municipal de Meio Ambiente - São José do Rio Preto',
'Olá Carlos! Você está no Sistema de Dados da Secretaria Municipal de Meio Ambiente - São José do Rio Preto': 'Olá Carlos! Você está no Sistema de Dados da Secretaria Municipal de Meio Ambiente - São José do Rio Preto',
'Online book': 'Livros online',
'Online examples': 'Exemplos online',
'or import from csv file': 'ou importar de um arquivo csv',
'Origin': 'Origem',
'Other Plugins': 'Outros Plugins',
'Other Recipes': 'Outras Receitas',
'Outros Nomes': 'Outros Nomes',
'Overview': 'Visão Geral',
'Passagem de veículos': 'Passagem de veículos',
'Password': 'Senha',
'Password changed': 'Senha alterada',
"Password fields don't match": 'Campos de senha não coincide',
'Password reset': 'Alterar Senha',
'Perimetro': 'Perimetro',
'Permission': 'Permissão',
'Permissions': 'Permissões',
'Plugins': 'Plugins',
'Podador Coleta': 'Podador Coleta',
'Porte': 'Porte',
'Porte ou espécie inadequada': 'Porte ou espécie inadequada',
'Porte Repor': 'Porte Repor',
'Powered by': 'Desenvolvido com',
'Predio': 'Predio',
'Preface': 'Prefácio',
'Prejuízo a rede de água/esgoto': 'Prejuízo a rede de água/esgoto',
'previous %s rows': '%s linhas anteriores',
'previous 100 rows': '100 linhas anteriores',
'Profile': 'Perfil',
'Profile updated': 'Perfil atualizado',
'Projetos e/ou atividades': 'Projetos e/ou atividades',
'Proprietario': 'Proprietario',
'Protocolo': 'Protocolo',
'Protocolo Anterior': 'Protocolo Anterior',
'pygraphviz library not found': 'biblioteca pygraphviz não encontrada',
'Python': 'Python',
'Qtd': 'Qtd',
'Qtd Poda1': 'Qtd Poda1',
'Qtd Poda2': 'Qtd Poda2',
'Qtd Poda3': 'Qtd Poda3',
'Qtd Poda4': 'Qtd Poda4',
'Qtd Repor': 'Qtd Repor',
'Qtd Ret1': 'Qtd Ret1',
'Qtd Ret2': 'Qtd Ret2',
'Qtd Ret3': 'Qtd Ret3',
'Qtd Ret4': 'Qtd Ret4',
'Query Not Supported: %s': 'A busca não suporta: %s',
'Query:': 'Consulta:',
'Quick Examples': 'Exemplos rápidos',
'RAM': 'RAM',
'RAM Cache Keys': 'RAM Cache Keys',
'Ram Cleared': 'Ram Limpa',
'RAM contains items up to **%(hours)02d** %%{hour(hours)} **%(min)02d** %%{minute(min)} **%(sec)02d** %%{second(sec)} old.': 'RAM contains items up to **%(hours)02d** %%{hour(hours)} **%(min)02d** %%{minute(min)} **%(sec)02d** %%{second(sec)} old.',
'Recipes': 'Receitas',
'Record': 'Registro',
'record does not exist': 'registro não existe',
'Record ID': 'ID do Registro',
'Record id': 'id do registro',
'Regiao': 'Regiao',
'Register': 'Registre-se',
'register': 'Registre-se',
'Registrar': 'Registrar',
'Registrar Bairro': 'Registrar Bairro',
'Registrar despacho_templat': 'Registrar despacho_templat',
'Registrar Especie': 'Registrar Especie',
'Registrar foto': 'Registrar foto',
'Registrar Laudo': 'Registrar Laudo',
'Registrar Modelo': 'Registrar Modelo',
'Registrar Requerimento': 'Registrar Requerimento',
'Registrar Requerimentos': 'Registrar Requerimentos',
'Registrar Retirada': 'Registrar Retirada',
'Registrar {}': 'Registrar {}',
'Registration identifier': 'Idenficador de registro',
'Registration key': 'Chave de registro',
'Registration needs verification': 'Registration needs verification',
'Remember me (for 30 days)': 'Mantenha-me logado (por 30 dias)',
'Requerente': 'Requerente',
'Request reset password': 'Request reset password',
'Reset Password': 'Reset Password',
'Reset Password key': 'Resetar chave de senha',
'Resources': 'Recursos',
'Restrição à passagem de pedestres': 'Restrição à passagem de pedestres',
'Retrieve Password': 'Retrieve Password',
'Risco à população, patrimônio': 'Risco à população, patrimônio',
'Role': 'Regra',
'Roles': 'Regras',
'Rows in Table': 'Linhas na tabela',
'Rows selected': 'Linhas selecionadas',
'Save model as...': 'Salvar modelo como...',
'Search': 'Busca',
'Search Pages': 'Buscar Páginas',
'Seja Bem Vindo': 'Seja Bem Vindo',
'Semantic': 'Semântico',
'Services': 'Serviço',
'Sign Up': 'Cadastrar',
'Sign up': 'Cadastrar',
'Sindispercao': 'Sindispercao',
'Sinpolinizacao': 'Sinpolinizacao',
'Sistema de Dados da Secretaria Municipal de Meio Ambiente - São José do Rio Preto': 'Sistema de Dados da Secretaria Municipal de Meio Ambiente - São José do Rio Preto',
'Size of cache:': 'Tamanho do cache:',
'Slug': 'Slug',
'Spreadsheet-optimised export of tab-separated content including hidden columns. May be slow': 'Spreadsheet-optimised export of tab-separated content including hidden columns. May be slow',
'Spreadsheet-optimised export of tab-separated content, visible columns only. May be slow.': 'Spreadsheet-optimised export of tab-separated content, visible columns only. May be slow.',
'Start building a new search': 'Criar nova busca',
'starts with': 'começa com:',
'state': 'estado',
'Statistics': 'Estatísticas',
'Stylesheet': 'Folha de estilo',
'Submenu': 'Submenu',
'submit': 'enviar',
'Submit': 'Enviar',
'Success!': 'Sucesso!',
'Support': 'Suporte',
'Sure you want to delete this object?': 'Está certo(a) que deseja apagar este objeto?',
'Table': 'Tabela',
'Table name': 'Nome da tabela',
'Tamanhomax': 'Tamanhomax',
'Tecnico': 'Tecnico',
'Telefone': 'Telefone',
'Telefone1': 'Telefone1',
'TestMenu': 'TestMenu',
'Texto do Modelo': 'Texto do Modelo',
'The "query" is a condition like "db.table1.field1==\'value\'". Something like "db.table1.field1==db.table2.field2" results in a SQL JOIN.': 'Uma "consulta" é uma condição como "db.tabela1.campo1==\'valor\'". Expressões como "db.tabela1.campo1==db.tabela2.campo2" resultam em um JOIN SQL.',
'The Core': 'O Motor',
'The output of the file is a dictionary that was rendered by the view %s': 'A saída do arquivo é um dicionário que foi apresentado pela visão %s',
'The Views': 'As views',
'This App': 'Esta aplicação',
'This email already has an account': 'Este email já tem uma conta',
'This is a copy of the scaffolding application': 'Isto é uma cópia da aplicação modelo',
'Time in Cache (h:m:s)': 'Tempo em Cache (h:m:s)',
'Timestamp': 'Timestamp',
'Tipo': 'Tipo',
'Tipo de Imóvel': 'Tipo de Imóvel',
'Tipo Imovel': 'Tipo Imovel',
'Tipofruto': 'Tipofruto',
'Title': 'Title',
'Too short': 'Muito curto',
'Traceback': 'Rastro do Erro',
'TSV (Spreadsheets)': 'TSV (Planilha)',
'TSV (Spreadsheets, hidden cols)': 'TSV (Planilha, colunas ocultas)',
'Twitter': 'Twitter',
'unable to parse csv file': 'não foi possível analisar arquivo csv',
'Unable to send email': 'Incapaz de enviar email',
'Unidade': 'Unidade',
'Update:': 'Atualizar:',
'Use (...)&(...) for AND, (...)|(...) for OR, and ~(...)  for NOT to build more complex queries.': 'Use (...)&(...) para AND, (...)|(...) para OR, e ~(...)  para NOT para construir consultas mais complexas.',
'User': 'Userário',
'User ID': 'ID do Usuário',
'User Voice': 'Opinião dos usuários',
'Username': 'Username',
'Username already taken': 'Username already taken',
'Users': 'Usuários',
'Valor': 'Valor',
'Value already in database or empty': 'Value already in database or empty',
'Value not allowed': 'Value not allowed',
'Value not in database': 'Value not in database',
'Verify Password': 'Verify Password',
'Videos': 'Vídeos',
'View': 'Visualização',
'View %(entity)s': 'View %(entity)s',
'View Page': 'View Page',
'Web2py': 'Web2py',
'Welcome': 'Bem-vindo',
'Welcome %(username)s! Click on the link %(link)s to verify your email': 'Welcome %(username)s! Click on the link %(link)s to verify your email',
'Welcome %s': 'Bem-vindo %s',
'Welcome to web2py': 'Bem-vindo ao web2py',
'Welcome to web2py!': 'Bem-vindo ao web2py!',
'Which called the function %s located in the file %s': 'Que chamou a função %s localizada no arquivo %s',
'Wiki Example': 'Wiki Example',
'Wiki Page': 'Wiki Page',
'Working...': 'Trabalhando...',
'XML': 'XML',
'XML export of columns shown': 'XML export of columns shown',
'You are successfully running web2py': 'Você está executando o web2py com sucesso',
'You are successfully running web2py.': 'Você está executando o web2py com sucesso.',
'You can modify this application and adapt it to your needs': 'Você pode modificar esta aplicação e adaptá-la às suas necessidades',
'You visited the url %s': 'Você acessou a url %s',
'Árvore morta/seca': 'Árvore morta/seca',
'Árvore senescente, debilitada por poda/pragas/parasitas': 'Árvore senescente, debilitada por poda/pragas/parasitas',
}
